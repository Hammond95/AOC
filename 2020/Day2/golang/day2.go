...
func main() {
	...
	for _, line := range lines {
		...
		minVal, _ := strconv.Atoi(string(strings.Split(boundaries, "-")[0]))
		maxVal, _ := strconv.Atoi(string(strings.Split(boundaries, "-")[1]))

		letter := strings.Split(lineSlice[0], " ")[1]
		password := strings.TrimSpace(lineSlice[1])
		occurrences := strings.Count(password, letter)

		if(minVal <= occurrences && occurrences <= maxVal){
